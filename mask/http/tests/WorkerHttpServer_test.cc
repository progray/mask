...
void onRequest(const HttpRequest& request, HttpResponse* response)
{
  if (request.path() == "/")
  {
    ...
    response->setBody("<html>"
                      "<head><title>Mask</title></head>"
                      "<body>"
                      "<a href=\"https://github.com/decimalbell/mask\"><h1>Mask</h1></a>"
                      "</body>"
                      "</html>");
  }
  ...
  else if (request.path() == "/helloworld")
  {
    response->setStatusCode(200);
    response->setReasonPhrase("OK");
...
template <>
void Worker<HttpServer>::run()
{
  eventloop_.reset(new muduo::net::EventLoop());
  server_.reset(new HttpServer(eventloop_.get(), addr_, name_,
...
int main(int argc, char* argv[])
{
  ...
  int numWorkers = 4;
  std::vector<boost::shared_ptr<Worker<HttpServer> > > wokers;

  for (int i = 0; i < numWorkers; i++)
  {
    boost::shared_ptr<Worker<HttpServer> > worker(new Worker<HttpServer>(
